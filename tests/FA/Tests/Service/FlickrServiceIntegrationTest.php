...
class FlickrServiceIntegrationTest extends ConfigTestCase
{
    ...
    public function testSearch()
    {
        ...
        $minUpload = $date->getTimestamp();

        $date->modify('+23 hours 59 minutes');
        $maxUpload = $date->getTimestamp();

        $options = array(
            'user_id' => $this->config['flickr.user.id'],
            'min_upload_date' => $minUpload,
            'max_upload_date' => $maxUpload,
            'extras' => 'url_sq, url_t, url_q',
        );

        $actual = $this->service->search($options);
        $this->assertEquals($this->getSearchResults(), $actual);
    }
